...
const storySteps = [
  { number: "01", title: "Start with the feeling", copy: "Say the loose version out loud. Rally hears the destination, the occasion, and the thing you actually want from the trip." },
  { number: "02", title: "Rally checks around", copy: "Your friends answer privately. Dates, budgets, energy levels, and strong opinions come back without another group-chat pileup." },
  { number: "03", title: "A few good directions", copy: "Rally does the research, makes the tradeoffs visible, and brings the group something worth reacting to." },
  { number: "04", title: "The plan keeps moving", copy: "When a decision is waiting, Rally nudges the right person. The planner gets to be in the trip again." },
];
  ...
  return (
    <main className="rally-v2-site">
      <nav className="v2-nav" aria-label="Main navigation">
        <Link className="v2-brand" href="/" aria-label="Rally home"><span className="v2-mark" aria-hidden="true"><i /><i /><i /></span>Rally</Link>
        ...
        <a className="v2-nav-cta" href={smsHref}>Text Rally <span aria-hidden="true">↗</span></a>
      </nav>
      ...
      <section className="v2-hero" aria-labelledby="v2-hero-title">
        <div className="v2-hero-copy">
          ...
          <p className="v2-hero-subtitle">Rally takes the trip from “we should” to “we’re going”—by text, with everyone’s preferences in the plan.</p>
        ...
        <div className="v2-hero-art" aria-label="Rally conversation preview">
          ...
          <div className="v2-phone v2-phone-hero">
            ...
            <div className="v2-phone-bar"><span>‹</span><strong>Rally<small>Trip concierge</small></strong><span>•••</span></div>
      ...
      <section className="v2-marquee" aria-label="Rally principles"><div><span>Less chasing</span><i>✳</i><span>More going</span><i>✳</i><span>Plans with a pulse</span><i>✳</i><span>Less chasing</span><i>✳</i><span>More going</span></div></section>

      <section className="v2-story" id="story" aria-labelledby="story-title">
        <div className="v2-story-intro"><p className="v2-kicker"><span /> The simple version</p><h2 id="story-title">A better way<br /><em>to make plans.</em></h2><p>There is always one person who becomes the trip planner by accident. Rally gives the work back to the group—and stays close enough to keep it moving.</p></div>
        <div className="v2-story-grid">
          ...
          <div className="v2-sticky-wrap"><div className="v2-sticky-card"><div className="v2-sticky-label">Rally, in the thread</div><div className="v2-sticky-quote">“I’ll ask around and bring you back something everyone can say yes to.”</div><div className="v2-sticky-line"><span /><small>gathering the good stuff</small></div><div className="v2-sticky-footer"><span>Rally</span><span>•••</span></div></div><p className="v2-sticky-caption">The group chat<br /><em>finally has a co-pilot.</em></p></div>
        </div>
      </section>

      <section className="v2-choice" id="try-it" aria-labelledby="choice-title">
        <div className="v2-choice-heading"><p className="v2-kicker"><span /> See yourself here</p><h2 id="choice-title">Start wherever<br /><em>you are.</em></h2><p>Every trip has a different first text. Pick one to see how Rally finds the thread.</p></div>
        ...
        <div className="v2-experience"><div className="v2-experience-copy"><p className="v2-kicker"><span /> {scene.label}</p><h3>{scene.headline}</h3><p>Rally turns a half-formed idea into a few directions with a point of view.</p><a className="v2-underlink" href={smsHref}>Try this with Rally <span>↗</span></a></div><div className="v2-carousel-card"><div className={`v2-carousel-image color-${card.color}`}><span>{card.title.slice(0, 1)}</span><small>RALLY / {String(cardIndex + 1).padStart(2, "0")}</small></div><div className="v2-carousel-details"><div><strong>{card.title}</strong><small>{card.detail}</small></div><div className="v2-carousel-controls"><button type="button" onClick={() => moveCard(-1)} aria-label="Previous trip direction">←</button><span>{cardIndex + 1} / {scene.cards.length}</span><button type="button" onClick={() => moveCard(1)} aria-label="Next trip direction">→</button></div></div></div></div>
      </section>

      <section className="v2-final"><p className="v2-kicker"><span /> Good trips take coordination</p><h2>Give Rally<br /><em>the group chat.</em></h2><a className="v2-button v2-button-coral" href={smsHref}>Text Rally <span>↗</span></a></section>
      <footer className="v2-footer"><Link className="v2-brand" href="/"><span className="v2-mark" aria-hidden="true"><i /><i /><i /></span>Rally</Link><span>Trips, handled.</span><Link href="/dashboard">Organizer view ↗</Link></footer>
    </main>
  );
}
