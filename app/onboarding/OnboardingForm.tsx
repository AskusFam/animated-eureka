...
export function OnboardingForm({ phoneNumber, tripId }: { phoneNumber: string; tripId?: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setMessage("Saved. Rally will use this when it plans for you.");
  }
  ...
  return (
    <form className="workspace-card profile-form" onSubmit={submit}>
      ...
      <section><div className="form-section-heading"><span>02</span><h2>Your travel energy</h2></div>
        <p className="field-note">Choose what sounds like you. Rally can adjust per trip.</p><div className="chip-list">{styles.map((style) => <button className={`chip ${selectedStyles.includes(style) ? "chip-selected" : ""}`} key={style} type="button" onClick={() => toggleStyle(style)}>{style}</button>)}</div>
      ...
      <section><div className="form-section-heading"><span>04</span><h2>How Rally should work</h2></div>
