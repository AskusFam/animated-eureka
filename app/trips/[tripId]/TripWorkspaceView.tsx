  ...
  return (
    <main className="workspace-shell">
      ...
      <div className="workspace-grid">
        <section className="workspace-card"><div className="card-kicker">Rally’s next move</div><h2>Align the group before the details pile up.</h2><p>Rally can collect private preferences, compare options, and bring the group a short recommendation when a decision is ready.</p><div className="workspace-actions"><a className="button" href={`/onboarding?phone=${encodeURIComponent(trip.participants.find((participant) => participant.role === "organizer")?.phoneNumber ?? "")}&tripId=${trip.id}`}>Set up your profile</a><button className="button secondary-button" type="button">Create review page</button></div></section>
        <section className="workspace-card"><div className="card-kicker">People</div><h2>{trip.participants.length} traveler{trip.participants.length === 1 ? "" : "s"}</h2><div className="participant-list">{trip.participants.map((participant) => <div className="participant-row" key={participant.id}><span className="avatar">{(participant.name ?? participant.phoneNumber).slice(0, 1).toUpperCase()}</span><span><strong>{participant.name ?? "Traveler"}</strong><small>{participant.role} · {participant.status}</small></span></div>)}</div><p className="field-note">Participants can reply privately by text. Rally keeps their preferences attached to this trip.</p></section>
      </div>
      <section className="workspace-card workspace-roadmap"><div><div className="card-kicker">Trip plan</div><h2>What Rally will handle</h2></div><div className="roadmap-list"><div><span>01</span><p><strong>Discover</strong>Research destinations and options around the group’s intent.</p></div><div><span>02</span><p><strong>Coordinate</strong>Collect preferences privately and nudge only when a decision is waiting.</p></div><div><span>03</span><p><strong>Recommend</strong>Bring back a concise comparison page for approval.</p></div><div><span>04</span><p><strong>Finalize</strong>Turn the approved direction into an itinerary.</p></div></div></section>
    </main>
  );
}
